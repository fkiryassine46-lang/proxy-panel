...
app = Flask(__name__)
app.secret_key = "change-me-if-you-want"  # for Flask session
...
<body>
  <div class="card">
    <div class="header-row">
      ...
      <div class="product-tag">Proxy panel · Local access</div>
    </div>
    <h1>{{ title }}</h1>
    <div class="subtitle">Sign in with your admin credentials.</div>
...
<body>
  <div class="shell">
    ...
    <div class="footer-note">
      Local-only admin panel · Source file: {{ stats.proxy_source }} · DB: {{ stats.db_file }}
    </div>
  </div>
</body>
</html>
    ...
    body = render_template_string("""
      ...
      <div class="grid">
        ...
        <div class="card">
          ...
          <div class="muted">Each client gets its own text file.</div>
        </div>
        <div class="card">
          <h3>Pool status</h3>
            ...
            fetch('{{ url_for("proxies_check_all") }}', {
              ...
              .then(data => {
                ...
                data.results.forEach(item => {
                  setStatus(item.proxy, item.status === 'ok' ? 'ok' : 'fail');
                });
              })
              .catch(err => {
                console.error(err);
