...
# true  = ne garder que les proxys dont PUBLIC_IP == IP_PPP
# false = garder tous les proxys qui répondent (même IP NAT différente)
STRICT_IP_MATCH=false
...
#     (STRICT_IP_MATCH controls behaviour)
###########################################

echo -e "Running initial proxy verification (keeping only ${GREEN}working${RESET} proxies)..."

TMP_FILE_OK=$(mktemp)
TMP_FILE_FAIL=$(mktemp)
MAX_HEALTH_JOBS=50   # parallel checks (tunable)

export GREEN RED ORANGE RESET TMP_FILE_OK TMP_FILE_FAIL STRICT_IP_MATCH
...
while IFS=':' read -r HOST PORT USER PASS; do
    ...
    (
        ...
        if [ -n "$PUBLIC_IP" ]; then
            if [ "$STRICT_IP_MATCH" = "true" ] && [ "$PUBLIC_IP" != "$HOST" ]; then
                # Répond mais IP différente -> marqué en mismatch, pas gardé
                echo "${HOST}:${PORT}:${PUBLIC_IP}" >>"$TMP_FILE_FAIL"
                echo -e "${HOST}:${PORT} -> ${RED}STATUS MISMATCH (${PUBLIC_IP}, REMOVED)${RESET}"
            else
                # Proxy OK (IP identique ou NAT accepté)
                echo "${HOST}:${PORT}:${USER}:${PASS}" >>"$TMP_FILE_OK"
                if [ "$PUBLIC_IP" = "$HOST" ]; then
                    echo -e "${HOST}:${PORT} -> ${GREEN}STATUS OK (KEPT)${RESET}"
                else
                    echo -e "${HOST}:${PORT} -> ${ORANGE}STATUS OK (NAT ${PUBLIC_IP}, KEPT)${RESET}"
                fi
            fi
        else
            # aucun retour d'api.ipify.org
            echo "${HOST}:${PORT}:${USER}:${PASS}" >>"$TMP_FILE_FAIL"
...
echo -e " ${GREEN}Working proxies kept${RESET} : ${EXPORT_OK}"
echo -e " ${RED}Removed (bad or mismatch)${RESET} : ${EXPORT_FAIL}"
