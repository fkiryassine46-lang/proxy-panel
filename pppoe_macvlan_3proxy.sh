...
ip rule show | awk '{print $1}' | sed 's/://g' | while read -r pref; do
  if [[ "$pref" =~ ^[0-9]+$ ]] && [ "$pref" -ge 4000 ] && [ "$pref" -le 20000 ]; then
...
proxy_test() {
  ...
  local url="http://${user}:${pass}@${host}:${port}"
  curl -sS --max-time "$VERIFY_TIMEOUT" -x "$url" https://www.google.com >/dev/null 2>&1
}

setup_policy() {
  local i="$1" ppp_if="$2" ip_ppp="$3"
  local table_id=$((TABLE_BASE + i))
  ip route flush table "$table_id" >/dev/null 2>&1 || true
  ip route add "${ip_ppp}/32" dev "$ppp_if" table "$table_id" >/dev/null 2>&1
  ip route add default dev "$ppp_if" table "$table_id" >/dev/null 2>&1
...
wait_ip() {
  local ppp_if="$1"
  local t=60
  while ! ip addr show "$ppp_if" >/dev/null 2>&1 && [ "$t" -gt 0 ]; do sleep 1; t=$((t-1)); done
  ip -4 addr show dev "$ppp_if" | awk '/inet /{print $2}' | cut -d/ -f1 | head -n1
}

###########################################
# MAIN: build stable pool
###########################################
echo
...
for i in $(seq 0 $((MAXTRY - 1))); do
  ...
  setup_policy "$i" "$ppp_if" "$ip_ppp" || continue

  # test internet through that IP via proxy port mapping (we map 1 port per stable PPP)
  port="$PORT_NEXT"
  if proxy_test "$ip_ppp" "$port" "fibre123" "fibrebebe123"; then
    ...
    echo -e " -> ${GREEN}STABLE${RESET} $ppp_if IP $ip_ppp PORT $port (stable=${STABLE}/${TARGET})"
  else
    # kill bad ppp session (not routed)
...
if [ "$STABLE" -lt "$TARGET" ]; then
  echo -e "${RED}Unable to reach TARGET=${TARGET}. Got STABLE=${STABLE}. Increase MAXTRY or FAI limit.${RESET}"
fi
...
###########################################
# WATCHDOG: auto-heal stable sessions
###########################################
echo "Starting watchdog (checks every 60s)..."
while true; do
  while IFS=':' read -r HOST PORT USER PASS; do
    [ -z "$HOST" ] && continue
    if ! proxy_test "$HOST" "$PORT" "$USER" "$PASS"; then
      echo -e "${RED}DOWN${RESET} ${HOST}:${PORT} -> will need rebuild (run script again to refill pool)"
    fi
