...
"""
Simple proxy management panel with modern dashboard UI.
...
ADMIN_PASSWORD = "lolopolo"  # local only, pas sécurisé pour Internet !

CHECK_TIMEOUT = 20  # secondes pour un proxy
USER_AGENT = (
...
def _load_clients() -> None:
    """Charge la base clients depuis le fichier JSON (si présent)."""
    global CLIENTS
...
def _load_proxies() -> None:
    """Lit /root/proxies.txt et re-construit la liste des proxys.

    On réapplique ensuite les assignations existantes depuis CLIENTS.
    """
    global PROXIES
    PROXIES = []
    ...
    # Réapplique les assignations depuis la base clients
...
def _proxy_to_requests_dict(proxy_str: str) -> Dict[str, str]:
    # proxy_str : ip:port / ip:port:user:pass
...
def _run_check_all():
    ...
    # Marque tout en "checking" d'abord
    for p in PROXIES:
        p["status"] = "checking"
...
# -----------------------------------------------------------------------------
# Templates (UI style “Purple dashboard”)
# -----------------------------------------------------------------------------
...
<head>
    ...
    <style>
        ...
        * {
        ...
        body {
            min-height: 100vh;
            background: radial-gradient(circle at top left, var(--bg-gradient-1), #1b1333),
        ...
        .btn {
            ...
            justify-content: center;
            gap: 6px;
            transition: transform 0.06s ease, box-shadow 0.12s ease, opacity 0.15s;
        }

        .btn-primary {
            background: linear-gradient(135deg, var(--accent), var(--accent-strong));
            color: #fff;
            box-shadow: 0 12px 25px rgba(88, 28, 135, 0.60);
        }
        ...
        .pill-fail {
            background: rgba(255, 75, 106, 0.10);
            color: var(--danger);
        }
        ...
        .flash-error {
            background: rgba(255, 75, 106, 0.06);
            color: var(--danger);
  ...
  <div class="login-card">
    <div style="display:flex;align-items:center;gap:10px;margin-bottom:14px;">
      ...
      <div>
        ...
        <div class="sidebar-logo-text-sub">Local admin access</div>
      </div>
    </div>

    <div class="login-title">Admin login</div>
    <div class="login-subtitle">Sign in to manage proxies and clients.</div>

    {% if error %}
      <div class="flash flash-error" style="margin-bottom:12px;">{{ error }}</div>
    {% endif %}
    ...
    <form method="post">
      ...
      <button class="btn btn-primary" style="width:100%;margin-bottom:4px;">Sign in</button>
    </form>

    <div class="login-footer">
      Local-only panel. Default admin: <strong>admin / lolopolo</strong>
    </div>
  </div>
</div>
{% endblock %}
...
def _render_page(active: str, content_html: str, page_title: str = "Dashboard"):
    """Injecte le layout + sidebar + topbar autour du contenu."""
    template = """
    {% extends base %}
    {% block body %}
    <div class="layout">
      <aside class="sidebar">
        ...
        <div class="sidebar-nav">
          ...
          <a href="{{ url_for('dashboard') }}" class="sidebar-link {% if active == 'dashboard' %}sidebar-link-active{% endif %}">
            <span>Dashboard</span>
            {% if active == 'dashboard' %}<span class="sidebar-pill">Live</span>{% endif %}
          </a>

          <a href="{{ url_for('proxies') }}" class="sidebar-link {% if active == 'proxies' %}sidebar-link-active{% endif %}">
            <span>Proxies</span>
            <span class="sidebar-pill">{{ totals.total }} total</span>
          </a>

          <a href="{{ url_for('clients') }}" class="sidebar-link {% if active == 'clients' %}sidebar-link-active{% endif %}">
            <span>Clients</span>
            <span class="sidebar-pill">{{ totals.clients }} users</span>
          </a>

          <a href="{{ url_for('settings') }}" class="sidebar-link {% if active == 'settings' %}sidebar-link-active{% endif %}">
            <span>Settings</span>
          </a>
        </div>
...
def login():
    ...
    return render_template_string(LOGIN_HTML, base=BASE_HTML, error=error)
...
def dashboard():
    ...
    <div class="content-grid">
      ...
      <div class="card">
        ...
        <div class="card-footer">
          Each client gets a dedicated export of proxies.
        </div>
      </div>
    </div>
    ...
    """
    return _render_page("dashboard", content, page_title="Dashboard")
...
def proxies():
    ...
    for p in PROXIES:
        ...
        rows_html.append(
            ...
            <tr>
              <td>{p['proxy']}</td>
              <td>{p['assigned_to'] or '<span class="muted">Unassigned</span>'}</td>
              <td><span class="pill {pill_class} status-pill" data-proxy-id="{p['id']}">{label}</span></td>
            </tr>
            """
        )
    ...
    <div class="content-grid">
      ...
      <div class="card">
        ...
        <div style="margin-top:8px;display:flex;gap:10px;">
          <button id="check-all-btn" class="btn btn-primary" onclick="startCheckAll()" type="button">
            Check ALL Proxies
          </button>
    ...
    function updateProgressView(data) {{
        ...
        bar.style.width = pct + '%';
        label.textContent = data.running ? ('Checking ' + done + '/' + total + ' (' + pct + '%)') :
                                          ('Last check: ' + done + '/' + total + ' (' + pct + '%)');
    ...
    async function pollStatus() {{
        try {{
            ...
            if (data.running) {{
                ...
                // recharge la page pour voir les statuts finaux
    ...
    // Au chargement, récupérer l'état éventuellement en cours
    (async function() {{
        try {{
            const resp = await fetch('{url_for("check_status")}');
            if (!resp.ok) return;
    ...
    """
    return _render_page("proxies", content, page_title="Proxies")
...
def clients():
    ...
    if request.method == "POST":
        ...
        if not name or count <= 0:
            ...
            if len(available) < count:
                ...
                # Génère le fichier texte à télécharger
                content = "\n".join(client["proxies"])
        ...
        <form method="post" style="margin-top:4px;">
          <div style="display:flex;gap:8px;align-items:flex-end;flex-wrap:wrap;">
            <div>
              ...
              <input name="name" class="field-input" style="background:#f9fafb;color:#111827;border-radius:10px;">
            </div>
            <div>
              <div class="field-label">Number of proxies</div>
              <input name="count" type="number" min="1" class="field-input"
                     style="width:120px;background:#f9fafb;color:#111827;border-radius:10px;">
            </div>
            <div>
    ...
    """
    return _render_page("clients", content, page_title="Clients")
...
def settings():
    if request.method == "POST":
        # pour l'instant, juste possibilité de changer le mot de passe admin
        new_pass = request.form.get("new_password", "").strip()
        if new_pass:
            global ADMIN_PASSWORD
            ADMIN_PASSWORD = new_pass
            flash("Admin password updated (only for this process run).", "success")
        else:
    ...
    <div class="card">
      ...
      <form method="post" style="max-width:420px;">
        ...
        <input type="password" name="new_password" class="field-input"
               style="background:#f9fafb;color:#111827;border-radius:10px;margin-bottom:10px;">
        <button class="btn btn-primary" type="submit">Update password</button>
      </form>
    </div>
    """
    return _render_page("settings", content, page_title="Settings")
